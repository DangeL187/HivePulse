...
def permissions_check_user_admin(admin_token, user_token):
    ...
    check('admin' in get_roles_admin(admin_token)["roles"], 'get roles admin')
    check('admin' in get_roles_user(user_token)["roles"], 'get roles user')
...
def main():
    ...
    print('\n[*] Device register and login...')

    register_device(user_token)

    device_access_token, device_refresh_token = login_device()
    check(device_access_token is not None and device_refresh_token is not None, 'login device')

    device_access_token = refresh_device(device_access_token)
    check(device_access_token is None, 'failed to refresh device with access token')

    device_access_token = refresh_device(device_refresh_token)
    ...
    res = revoke_role_operator_from_user(2, admin_token)
